...
def save_results(results, name, test_names):
    results = pd.DataFrame(results, columns=['subject_number'] + test_names)
    results.to_csv(pathes.result_path + name + ".csv", index=None)
