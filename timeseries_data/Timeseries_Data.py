...
class TimeseriesData():
    ...
    def __init__(self, data=0, header_size=0, num_of_ts=0, ts_names=0):
        ...
        self.header_col_names = data.columns[:self.header_size]
    ...
    def read_from_csv(self, path):
        ...
        self.ts_length = (self.data.shape[1] - self.header_size) // self.num_of_ts
        self.header_col_names = df.columns[: self.header_size]
    ...
    def to_csv(self, path):
        ...
        # insert metadata and header col names into dataframe header
        for i in range(self.header_size):
            col_name = data.columns.to_list()[i]
            if i == 0:
                data.rename(columns={col_name: metastring}, inplace=True)
            else:
                data.rename(columns={col_name: self.header_col_names[i]}, inplace=True)
